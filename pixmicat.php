...
function updatelog($resno=0,$page_num=-1,$single_page=false){
	...
	$pte_vals = array('{$THREADFRONT}'=>'','{$THREADREAR}'=>'','{$SELF}'=>PHP_SELF,
		...
		'{$DEL_PASS_FIELD}' => '<input type="password" name="pwd" size="8" value="" id="delpass"/>',
		'{$DEL_SUBMIT_BTN}' => '<button class="btn waves-effect waves-light" type="submit">'._T('del_btn').'</button>');
	...
	for($page = $page_start; $page <= $page_end; $page++){
		...
		if($resno){ // 回應分頁
			if(RE_PAGE_DEF > 0){ // 回應分頁開啟
				$pte_vals['{$PAGENAV}'] .= '<table class="bordered" ><tr><td style="white-space: nowrap;">';
			...
			$pte_vals['{$PAGENAV}'] .= '<table class="bordered" ><tr>';
			if($prev >= 0){
...
function valid(){
	...
	$dat .= '<div id="banner">'.$links.'<div class="bar_admin"><h5>'._T('admin_top').'</h5></div>
</div>
...
';
	...
	if(!$isCheck){
		echo '<br />
<p><input type="radio" name="admin" value="del" checked="checked" id="del"/><label for="del">'._T('admin_manageposts').'</label></p>
<p><input type="radio" name="admin" value="optimize" id="optimize"/><label for="optimize">'._T('admin_optimize').'</label></p>
<p><input type="radio" name="admin" value="check" id="check"/><label for="check">'._T('admin_check').'</label></p>
<p><input type="radio" name="admin" value="repair" id="repair"/><label for="repair">'._T('admin_repair').'</label></p>
<p><input type="radio" name="admin" value="export" id="export"/><label for="export">'._T('admin_export').'</label></p><br />
<input type="hidden" name="mode" value="admin" id="admin"/>
<input type="password" name="pass" size="8" />
<button class="btn waves-effect waves-light" type="submit">'._T('admin_verify_btn').'</button>
</div>
</form>';
		die("\n</body>\n</html>");
...
<p><input type="radio" name="admin" value="check" id="check"/><label for="check">'._T('admin_check').'</label></p>
<p><input type="radio" name="admin" value="repair" id="repair"/><label for="repair">'._T('admin_repair').'</label></p>
<p><input type="radio" name="admin" value="export" id="export"/><label for="export">'._T('admin_export').'</label></p>
<p><input type="radio" name="admin" value="logout" id="logout"/><label for="logout">'._T('admin_logout').'</label></p><br />
<input type="hidden" name="mode" value="admin" />

<button class="btn waves-effect waves-light" type="submit" name="action">'._T('admin_submit_btn').'</button>
</div>
</form>';
		die("\n</body>\n</html>");
	}
...
<th style="text-align:center">$modFunc</th><th style="text-align:center">$THstop</th><th><p><input type="checkbox" name="clist[]" value="$no" id="cl$no"/><label for="cl$no">$no</label></p></th><td><small>$now</small></td><td>$sub</td><td><b>$name</b></td><td><small>$com</small></td><td>$host</td><td style="text-align:center">$clip ($size)<br />$md5chksum</td>
</tr>
...
<select name="func"><option value="delete">'._T('admin_delete').'</option>';
	...
	echo '</select>
<button class="btn waves-effect waves-light" type="submit">'._T('admin_submit_btn').'</button> <button class="btn waves-effect waves-light" type="reset">'._T('admin_reset_btn').'</button> <input type="checkbox" name="onlyimgdel" id="onlyimgdel" value="on" /><label for="onlyimgdel">'._T('del_img_only').'</label></p>
...
function search(){
	...
	$dat .= '<div id="banner"><div class="bar_admin"><h5>'._T('search_top').'</h5></div>
</div>
';
	echo $dat;
	if($searchKeyword==''){
...
';
		echo '<ul>'._T('search_notice').'<div class="row"><div class="input_field col s12"><label for="searchtxt">關鍵字</label><input type="text" name="keyword" size="30" id="searchtxt"/></div></div>
<div class="row"><div class="input_field col s12"><label>'._T('search_target').'</label><select name="field"><option value="com" selected>'._T('search_target_comment').'</option><option value="name">'._T('search_target_name').'</option><option value="sub">'._T('search_target_topic').'</option><option value="no">'._T('search_target_number').'</option></select></div></div>
<div class="row"><div class="input_field col s12"><label>'._T('search_method').'</label><select name="method"><option value="AND" selected="selected">'._T('search_method_and').'</option><option value="OR">'._T('search_method_or').'</option></select></div></div>
<button class="btn waves-effect waves-light" type="submit">'._T('search_submit_btn').'</button>
</li>
</ul>
</div>
</form>';
...
function showstatus(){
	...
	$dat .= '<div id="banner">'.$links.'<div class="bar_admin"><h5>'._T('info_top').'</h5></div>
</div>
';

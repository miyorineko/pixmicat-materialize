...
function form(&$dat, $resno, $iscollapse=true, $retURL=PHP_SELF, $name='', $mail='', $sub='', $com='', $cat='', $mode='regist'){
	...
	if(USE_FLOATFORM && !$resno && $iscollapse) $pte_vals['{$FORMTOP}'] .= "\n".'<div class="row"><span id="show" class="hide center-align" onmouseover="showform();" onclick="showform();">['._T('form_showpostform').']</span><span id="hide" class="show right-align" onmouseover="hideform();" onclick="hideform();">['._T('form_hidepostform').']</span></div>';
	$pte_vals += array('{$MAX_FILE_SIZE}' => MAX_KB * 1024,
		...
		'{$FORM_SUBMIT}' => '<a href="#" onclick="document.getElementById(\'postform_main\').submit()" name="sendbtn">'._T('form_submit_btn').'</a>',
		'{$FORM_COMMENT_TEXT}' => _T('form_comment'),
		'{$FORM_COMMENT_FIELD}' => '<textarea class="materialize-textarea" maxlength="'.COMM_MAX.'" name="'.FT_COMMENT.'" id="fcom" cols="48" rows="4">'.$com.'</textarea><textarea name="com" class="hide" cols="48" rows="4">EID OG SMAPS</textarea>',
		'{$FORM_DELETE_PASSWORD_FIELD}' => '<input type="password" name="pwd" size="8" maxlength="8" value="" id="pwd"/>',
	...
	if(USE_CATEGORY) {
		$pte_vals += array('{$FORM_CATEGORY_FIELD}' => '<input id="category" type="text" name="category" size="28" value="'.$cat.'" />',
...
function foot(&$dat){
	...
	$pte_vals['{$FOOTER}'] .= '<small class="right">- <a rel="nofollow noreferrer license" href="http://php.s3.to" target="_blank">GazouBBS</a> + <a rel="nofollow noreferrer license" href="http://www.2chan.net/" target="_blank">futaba</a> + <a rel="nofollow noreferrer license" href="http://pixmicat.openfoundry.org/" target="_blank">Pixmicat!</a> -</small>';
